...
#   export GEN_PROVIDER="openai"                 # or "gemini" or leave unset for extractive fallback
#   export GEN_MODEL="gpt-4.1-mini"
#   export EMBED_PROVIDER="openai"               # "openai" recommended (no torch needed)
...
# Retrieval
MAX_RESULTS = int(os.environ.get("MAX_RESULTS", "20"))          # UI evidence list limit
...
# Embeddings provider
EMBED_PROVIDER = os.environ.get("EMBED_PROVIDER", "openai").strip().lower()  # "openai" recommended
...
def _embed_hashing(texts: List[str], dim: int = 1024) -> np.ndarray:
    """
    Lightweight local fallback (no torch / no sklearn):
    - bag-of-words hashing
    - cosine-ready via L2 normalization
    """
...
# -------------------- Manual tagging helpers --------------------
def parse_tags_csv(s: str) -> List[str]:
    """
    Comma-separated tags -> list of unique tags (case-insensitive uniqueness),
...
def parse_file_tags_spec(spec: str) -> Tuple[Dict[str, List[str]], List[Tuple[str, List[str]]]]:
    """
    Per-file tags spec, one mapping per line:
      - exact file:   path/to/file.pdf | tag1, tag2
      - exact file:   path/to/file.pdf: tag1, tag2
      - prefix rule:  path/to/subfolder/ | tag1, tag2   (applies to any relpath starting with that prefix)

    Returns (exact_map, prefix_rules)
    """
    ...
    for raw in (spec or "").splitlines():
        ...
        key = sanitize_relpath(key.strip())
        tags = parse_tags_csv(val.strip())
        if not key or not tags:
            continue

        if key.endswith("/"):
            prefixes.append((key, tags))
        else:
            exact[key] = merge_tags(exact.get(key, []), tags)

    # longest prefixes first (more specific matches first)
...
def index_one_file(dataroom: str, relpath: str, abs_path: str, tags: List[str]) -> Tuple[str, int]:
...
def reindex_all_uploads() -> Tuple[int, int]:
    """
    Rebuild Chroma + per-file CSVs from uploads/, preserving manual tags from existing files.json.
    """
    old_db = load_files_db()
    old_tags_by_key: Dict[Tuple[str, str], List[str]] = {}
    for rec in (old_db.get("files", {}) or {}).values():
        room = (rec.get("dataroom") or "default").strip() or "default"
        relp = sanitize_relpath(rec.get("relpath") or "")
        tags = rec.get("tags") or []
        if relp:
            old_tags_by_key[(room, relp)] = [t for t in tags if isinstance(t, str)]
    ...
    for dataroom, relpath, abs_path in items:
        try:
            tags = old_tags_by_key.get((dataroom, relpath), [])
            _, n = index_one_file(dataroom, relpath, abs_path, tags=tags)
            if n > 0:
                files_indexed += 1
                chunks_indexed += n
        except Exception:
...
# -------------------- Generation instructions (conservative) --------------------
GEN_INSTRUCTIONS = (
...
# -------------------- HTML (RAG-only search + manual tagging) --------------------
HTML = """
<!doctype html>
<html>
  <head>
    ...
    <div class="grid">
      <div class="card">
        ...
        <form method="get" action="/">
          <input name="q" type="text" placeholder="Ask a question across your dataroom..." value="{q}" />
          ...
          <input name="tag" type="text" placeholder="Optional tag filter" value="{tag}" />

          <button type="submit">Search</button>
        </form>

        <div class="hint">
          Endpoints: <code>/api/search</code>, <code>/api/files</code>.
          {llm_hint}
        </div>
      ...
      <div class="card">
        <h2>Upload (manual tags)</h2>
        <div class="small">
          Files are stored under <span class="mono">uploads/&lt;dataroom&gt;/...</span>. Tags are manual only.
        </div>
        ...
        <form method="post" action="/upload" enctype="multipart/form-data" class="stack" style="margin-top:.75rem;">
          ...
          <div class="row">
            <label>Folder tags (apply to every uploaded file)</label>
            <input name="folder_tags" type="text" placeholder="e.g., buyout, healthcare" />
          </div>

          <div class="stack">
            <label>Per-file or subfolder tags (optional)</label>
            <textarea name="file_tags" placeholder="Examples:
# exact file
IC/teaser.pdf | teaser, buyout
# subfolder prefix rule (trailing slash)
IC/ | buyout"></textarea>
            <div class="small">Format: <span class="mono">relpath | tag1, tag2</span> or <span class="mono">relpath: tag1, tag2</span>. Use <span class="mono">folder/</span> to tag all files under that prefix.</div>
          </div>

          <div class="stack">
            <label>Select files or a folder</label>
            <input name="files" type="file" accept=".pdf,.docx" multiple />
            <input name="folder" type="file" accept=".pdf,.docx" webkitdirectory directory multiple />
            <div class="small">Use files picker for individual documents; folder picker for bulk upload.</div>
          </div>
        ...
        <div class="row" style="margin-top:.9rem;">
          ...
          <span class="small">Rebuilds Chroma + CSVs from everything under <span class="mono">uploads/</span>, preserving tags from <span class="mono">files.json</span>.</span>
        </div>
...
def render_ask_block(q: str, rag: Dict) -> str:
    if not q.strip():
        return "<p class='small'>Enter a question, then click Search.</p>"
    ...
    topline = ""
    if low_conf:
        topline = f"<div class='msg'>Low-confidence retrieval: best score {best_score:.4f} &lt; {RAG_MIN_SCORE:.2f}. Showing conservative output and excerpts.</div>"

    ev_lis = []
    for i, r in enumerate(evidence[:MAX_RESULTS], start=1):
...
def index():
    ...
    if q.strip():
        ...
        results_block = "<p class='small'>Type a question above. Search always runs RAG (answer + evidence).</p>"
...
def upload():
    ...
    folder_tags = parse_tags_csv(request.form.get("folder_tags") or "")
    file_tags_spec = request.form.get("file_tags") or ""
    exact_map, prefix_rules = parse_file_tags_spec(file_tags_spec)
    ...
    for f in uploads:
        raw_name = f.filename  # folder uploads include relative paths
        relpath_in = sanitize_relpath(raw_name)
        if not allowed_ext(relpath_in):
            skipped += 1
            continue

        # compute file-specific tags:
        # 1) exact relpath match
        # 2) exact basename match (convenience)
        # 3) any prefix rule matches
        file_specific: List[str] = []
        if relpath_in in exact_map:
            file_specific = merge_tags(file_specific, exact_map.get(relpath_in, []))
        base = sanitize_relpath(os.path.basename(relpath_in))
        if base in exact_map:
            file_specific = merge_tags(file_specific, exact_map.get(base, []))
        for pref, tags in prefix_rules:
            if relpath_in.startswith(pref):
                file_specific = merge_tags(file_specific, tags)

        combined_tags = merge_tags(folder_tags, file_specific)

        # ensure unique path inside room_dir
        relpath_unique = ensure_unique_path(room_dir, relpath_in)
        ...
        try:
            _, n = index_one_file(dataroom, relpath_unique, abs_path, tags=combined_tags)
            if n > 0:
                indexed_files += 1
                indexed_chunks += n
            else:
